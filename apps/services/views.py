...
from apps.services.models import Service, ServiceTokens, SchedulePolicy, Token, ServiceSilenced
from apps.incidents.models import Incident
from apps.openduty.tasks import unsilence_service
from apps.openduty.event_log import EventLog
...
def token_delete(request, token_id):
    try:
        ...
        return HttpResponseRedirect(
            reverse('openduty.services.edit', None, [str(token.service_id.id)])
        )
    except Service.DoesNotExist:
        raise Http404
...
def token_create(request, service_id):
    try:
        ...
        token = Token()
        token.save()
        service_token = ServiceTokens.objects.create(
            service_id=service, token_id=token, name=request.POST['key_name']
        )
        service_token.save()
